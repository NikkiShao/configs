...
const float TRAIL_BLEND = 0.5;
const float ACCENT_BLEND = 0.35;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float mod = .003;
    //trailblaze
    vec4 trailTint = mix(TRAIL_COLOR_ACCENT, fragColor, 1. - smoothstep(0., sdfTrail + mod, 0.003));
    trailTint = mix(TRAIL_COLOR, trailTint, 1. - smoothstep(0., sdfTrail + mod, 0.002));
    trailTint = mix(trailTint, TRAIL_COLOR, step(sdfTrail + mod, 0.));
    vec4 trail = mix(fragColor, trailTint, TRAIL_BLEND);
    //cursorblaze
    vec4 cursorTint = mix(TRAIL_COLOR_ACCENT, trail, 1. - smoothstep(0., sdfCurrentCursor + .001, 0.002));
    cursorTint = mix(TRAIL_COLOR, cursorTint, 1. - smoothstep(0., sdfCurrentCursor + .001, 0.002));
    trail = mix(trail, cursorTint, ACCENT_BLEND);
